...
def initialize_tracks(grouped_measurements):
    tracks = []
    track_ids = []
    for group in grouped_measurements[0]:
        track_id = len(tracks)
        tracks.append([group])
...
def generate_clusters(tracks, measurements, cov_inv):
    clusters = {i: [] for i in range(len(tracks))}
    new_track_start = len(tracks)
    for measurement in measurements:
        assigned = False
        for track_id, track in enumerate(tracks):
            if chi_squared_test(measurement, track, cov_inv):
                ...
                break
        if not assigned:
            clusters[new_track_start] = [measurement]
