...
def update_filter_with_max_probability(hypotheses, probabilities, clusters, kalman_filter):
    ...
    for track_id, report_idx in best_hypothesis:
        ...
        updated_states.append((kalman_filter.Meas_Time, 
                               np.linalg.norm(kalman_filter.Sf[:3]), 
                               np.degrees(np.arctan2(kalman_filter.Sf[1], kalman_filter.Sf[0])), 
                               np.degrees(np.arctan2(kalman_filter.Sf[2], np.sqrt(kalman_filter.Sf[0]**2 + kalman_filter.Sf[1]**2)))))
